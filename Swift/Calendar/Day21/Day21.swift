...
//  Day21
//
//  Created by Vyacheslav Khorkov on 21/12/2018.
...
struct Day21: Solution {
    func readInput(from path: String) -> (bound: Int, instructions: [(String, Instruction)]) {
        let input = Frog(path)
    ...
    func silver(_ input: (bound: Int, instructions: [(String, Instruction)])) -> Int {
        return run(input, [1024276, 19, 257, 65536, 256, 14339185])
    }
    
    func gold(_ input: (bound: Int, instructions: [(String, Instruction)])) -> Int {
        //   #ip 1
    ...
    private func run(_ input: (bound: Int, instructions: [(String, Instruction)]),
                     _ registers: [Int],
                     debugMode: Bool = false) -> Int {
        var instructions = input.instructions
        var registers = registers
        var pointer = registers[input.bound]
        var step = 0
        while instructions.indices ~= pointer {
            registers[input.bound] = pointer
