...
struct Day12: Solution {
    func readInput(from path: String) -> (line: [Int], map: [Int]) {
        let lines = Frog(path).readLines().lazy
            .filter { !$0.isEmpty }
            .map { $0.replacingOccurrences(of: "#", with: "1") }
            .map { $0.replacingOccurrences(of: ".", with: "0") }
        
        let initLine = lines[0].components(separatedBy: " ").last!
        let initial = initLine.map(String.init).compactMap(Int.init)
    ...
    func silver(_ input: (line: [Int], map: [Int])) -> Int {
        return run(NSMutableArray(array: input.line), input.map, 20)
    }
    
    func gold(_ input: (line: [Int], map: [Int])) -> Int {
        return run(NSMutableArray(array: input.line), input.map, 50000000000)
    }
    
    private func run(_ line: NSMutableArray, _ map: [Int], _ maxGen: Int) -> Int {
